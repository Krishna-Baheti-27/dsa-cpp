...
// for directed graph edge between u and v doesnt mean there would be edge between v and u, although there could be so directed graph can have bidirectional edges 
...
// here the number of edges is not related to number of nodes in any way, we can decrease m or increase m according to our choice, since graph is simply a set of nodes and edges and there is no other constraint
