...
// here the level zero means nodes at 0 distance from startign node, level 1 means nodes at 1 distance from starting node and so on and thats why its BFS, since we go breadth first increasing distance by 1 unit in each iteration, here is the distance is measured radially outwards, in all directions
...
// this code only traverses one component of a disconnected graph, for all traversing entire graph split into multiple components, use a for loop across all the nodes and apply this traversal for the nodes which are not visited
...
vector<int> bfsOfGraph(int v, vector<int> adj[]) {  // v means number of nodes or vertices
    ...
    while(!q.empty()) {
        ...
        for(auto adjNode : adj[node]) { // adj[node] is list of all neighbours of node
            ...
            if(!vis[adjNode]) { // if not visited then visit adjNode by inserting into queue
...
// O(N) time for initialising visited + O(N) time for which queue will run + O(2E) total time for which inner loop will run across the entire traversal, as we know its visiting all the degree of nodes so total 2E 
