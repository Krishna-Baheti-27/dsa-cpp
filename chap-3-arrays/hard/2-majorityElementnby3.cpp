...
vector<int> majorityElementBrute(const vector<int> &arr) {
    ...
    for(int i = 0; i < arr.size(); i++) {
        ...
        // check to ensure we are not inserting duplicates
...
// the optimal solution again uses the moore voting algo which we discused in majority element > n / 2 times, but here we can have two majority elements hence some conditions change

// we have to make sure that el1 and el2 are unique integers and when count1 == 0 then we check if arr[i] != other element becuase if it is then this should not be executed
...
vector<int> majorityElementOptimal(const vector<int> &arr) {
    ...
    for(int i = 0; i < arr.size(); i++) {
        ...
        if(count1 == 0 && arr[i] != ans2) {
            count1 = 1; // it was cancelled so reset count1
            ans1 = arr[i]; // try new element
            ...
            // if the element is none of our majority then we de crement both
            count1--;
            count2--;
        }
    }
