...
int timeToBurn(Node *root, Node *target) {
    ...
    while(!q.empty()) {
        ...
        // the for loop is necessary since at each time we ignite the entire level simultaneously not each node one by one
...
// No Increment if Fire Doesn't Spread: You only increment the time when you successfully "burn" a new level of nodes. Suppose if we are at our last level and fire has already spread and we find no new, unburnt neighbors, it means the fire has reached every possible node (we have completely burn the entire tree). The process is over. Incrementing the time again would be incorrect because the fire didn't advance.
