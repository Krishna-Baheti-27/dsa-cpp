...
int peakElBrute(const vector<int> &arr) {
    ...
    // edge cases, if these are picks then our work is easy we return early
    ...
    return -1; // would be executed if no peak but that is not possible, there will always be a peak since on both ends of array we negative infinity
} // O(N) time
...
int peakElOptimalSingle(const vector<int> &arr) {
    ...
    while(low <= high) {
        ...
        if(arr[mid] > arr[mid - 1]) { // mid lies on the increasing slope
